...
mkdir -p "$PROJECT_FULL/specs" "$PROJECT_FULL/changes/archive"
...
if [[ "$TARGET" == "codex" || "$TARGET" == "all" ]]; then
  ...
  mkdir -p "$CODEX_HOME_DIR/prompts" "$CODEX_HOME_DIR/skills/demandspec"
  cp "$KIT_ROOT"/commands/codex/*.md "$CODEX_HOME_DIR/prompts/"
  cp -R "$KIT_ROOT"/skills/demandspec/. "$CODEX_HOME_DIR/skills/demandspec/"
  cp "$KIT_ROOT/rules/AGENTS.md" "$PROJECT_FULL/AGENTS.md"
  echo "Installed Codex prompts to $CODEX_HOME_DIR/prompts"
  echo "Installed Codex skill to $CODEX_HOME_DIR/skills/demandspec"
fi
